-- =============================================================================
-- VoteChain — Consolidated Migration
-- Generated: 2026-05-28, merged through 2026-07-09
-- All prior sub-migrations are folded into this single file.
-- Execute once on a fresh Supabase database.
-- =============================================================================
...
begin
  ...
  if not exists (select 1 from pg_type where typname = 'tx_action_type') then
    create type app.tx_action_type as enum (
      ...
      'phase_transition',
      'commit_vote',
      'commit_vote_server_signed',
      'reveal_vote',
      'reveal_vote_server_signed',
      'suspend_space',
      'unsuspend_space',
      'terminate_space'
    );
...
-- ─── Admin Registry ─────────────────────────────────────────────────────────
-- access_scope defaults to 'specific' (changed from 'all' on 2026-07-05).
-- Existing 'all' rows are updated to 'specific' at the end of this migration.

create table if not exists app.admin_registry (
  email text primary key,
  assigned_role app.app_role not null default 'admin',
  organization_name text,
  access_scope text not null default 'specific',
  status text not null default 'pending',
  description text,
  created_by uuid references app.app_profiles(id) on delete set null,
  ...
  activation_accepted_at timestamptz,
  faculty text,
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
  ...
  constraint admin_registry_wallet_format check (wallet_address is null or wallet_address ~ '^0x[a-fA-F0-9]{40}$')
);
...
-- ─── User Wallets (deterministic key derivation) ────────────────────────────
...
create index if not exists idx_proposal_documents_proposal_draft_id on app.proposal_documents(proposal_draft_id, created_at desc);
...
-- role_for_email: handles voter (pending|active), admin (active), super_admin (active)
...
-- Used by hubungkan-dompet page to detect admin context before login.
...
-- can_manage_proposal: centralized check for scoped admin access
...
on app.app_profiles for select
...
on app.app_profiles for insert
with check (
  user_id = auth.uid()
...
on app.app_profiles for update
using (
  user_id = auth.uid()
...
-- proposal_drafts (restricted access model)
...
on app.proposal_drafts for select
using (
  app.has_role(array['super_admin'::app.app_role])
  or (
    app.has_role(array['admin'::app.app_role])
...
on app.proposal_drafts for insert
with check (
...
on app.proposal_drafts for update
using (
  app.has_role(array['super_admin'::app.app_role])
  or (
    app.has_role(array['admin'::app.app_role])
...
-- proposal_candidates (scoped admin via can_manage_proposal)
...
on app.proposal_documents for select
using (
  exists (
    select 1
    from app.proposal_drafts d
...
on app.proposal_documents for insert
with check (
...
-- whitelist_entries (scoped admin)
...
on app.proposal_whitelist_entries for all
using (app.can_manage_proposal(proposal_draft_id))
with check (app.can_manage_proposal(proposal_draft_id));

-- space_metadata_versions
create policy "metadata_versions_select_owner_or_admin"
on app.space_metadata_versions for select
using (
  exists (
    select 1
    from app.proposal_drafts d
...
on app.space_metadata_versions for insert
with check (
  exists (
    select 1
    from app.proposal_drafts d
...
on app.space_registry_map for select
using (
  exists (
    select 1
    from app.proposal_drafts d
...
on app.tx_audit_log for select
using (
  wallet_address in (
...
on app.tx_audit_log for insert
with check (
...
on app.tx_audit_log for update
using (
  wallet_address in (
    select wallet_address from app.app_profiles where user_id = auth.uid()
  )
...
on app.proof_exports for select
using (
...
on app.proof_exports for insert
with check (
  owner_profile_id = app.current_profile_id()
  or app.has_role(array['admin'::app.app_role, 'super_admin'::app.app_role])
);
...
on app.ops_audit_log for select
...
on app.notification_jobs for select
using (
...
on app.notification_jobs for insert
with check (
...
on indexer.indexer_sync_status for select
...
on app.admin_registry for select
using (
  app.has_role(array['super_admin'::app.app_role])
...
for all using (app.has_role(array['super_admin'::app.app_role]));

-- master_voters (admin-only SELECT — fixed 2026-07-09)
drop policy if exists "master_voters_select_auth" on app.master_voters;
drop policy if exists "master_voters_select_admin" on app.master_voters;
create policy "master_voters_select_admin"
on app.master_voters for select
using (
  app.has_role(array['admin'::app.app_role, 'super_admin'::app.app_role])
);

create policy "master_voters_insert_admin"
on app.master_voters for insert
with check (
  app.has_role(array['admin'::app.app_role, 'super_admin'::app.app_role])
);

create policy "master_voters_update_admin"
on app.master_voters for update
using (
  app.has_role(array['admin'::app.app_role, 'super_admin'::app.app_role])
)
with check (
...
on app.master_voters for delete
using (
  app.has_role(array['admin'::app.app_role, 'super_admin'::app.app_role])
);
...
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('public-assets', 'public-assets', true, 10485760, array['image/jpeg', 'image/png', 'image/webp'])
on conflict (id) do update
set public = excluded.public,
    file_size_limit = excluded.file_size_limit,
    allowed_mime_types = excluded.allowed_mime_types;
...
-- ─── Trigger: profile activation notification ───────────────────────────────
...
-- ─── Post-migration: fix access_scope defaults ──────────────────────────────
-- Changed from 'all' to 'specific' on 2026-07-05.
-- Existing 'all' rows updated to 'specific' so superadmin must re-assign.

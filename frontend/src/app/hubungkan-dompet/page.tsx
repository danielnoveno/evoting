            ...
            <AuthCard className="flex max-h-[84vh] max-w-none flex-col overflow-hidden border-slate-200/60 bg-white/90 p-0 shadow-[0_20px_50px_rgba(15,23,42,0.1)] backdrop-blur-md">
                ...
                <div className="flex flex-shrink-0 flex-col items-center px-2 pt-4">
                  ...
                  <div className="mt-4 text-center">
                    <h1 className="text-[26px] font-semibold tracking-tight text-slate-900 md:text-[30px]">Masuk Sistem</h1>
                    <p className="mx-auto mt-1 max-w-[300px] text-[14px] leading-relaxed text-slate-500">
                      Sambungkan wallet kampus Anda.
                    </p>
                  </div>
                </div>
                ...
                <div className="custom-scrollbar mt-6 flex-1 overflow-y-auto px-2 pb-6">
                  <StaggerContainer stagger={100} className="space-y-10">
                    ...
                    <div className="flex flex-col gap-6 border-b border-slate-100 pb-10">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`flex h-9 w-9 items-center justify-center rounded-xl ${isConnected ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}>
                        ...
                        {isConnected && <CheckCircle2 className="h-6 w-6 text-emerald-600" />}
                      </div>

                      {!isConnected ? (
                        <div className="flex flex-col gap-4">
                          <p className="text-[13px] leading-6 text-slate-600">
                            Gunakan wallet untuk masuk atau daftar.
                          </p>
                          <ConnectWallet 
                            className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#0F172A] px-5 text-[14px] font-semibold text-white transition-all hover:bg-[#1E293B] hover:shadow-lg hover:shadow-slate-200 active:scale-[0.98]"
                          >
                            Hubungkan Dompet
                    ...
                    <div className={`flex flex-col gap-6 transition-opacity duration-300 ${!isConnected ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`flex h-9 w-9 items-center justify-center rounded-xl ${authSession ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-600'}`}>
                            <Building2 className="h-5 w-5" />
                          </div>
                          <div>
                            <h2 className="text-[15px] font-bold text-slate-900">2. Verifikasi Mahasiswa</h2>
                            <p className="text-[12px] text-slate-500">Verifikasi akun Microsoft @uajy.ac.id</p>
                          </div>
                        </div>
                        {authSession && <CheckCircle2 className="h-6 w-6 text-blue-600" />}
                      </div>
                        ...
                        <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-4">
                          <div className="flex items-center justify-between gap-3 min-w-0 w-full">
                            <div className="flex items-center gap-3 min-w-0 flex-1">
                              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white shadow-sm text-blue-600">
                      ...
                      <div className="mt-2 pb-2">
                        <div className="rounded-2xl bg-[#0F172A] p-8 text-white shadow-2xl shadow-slate-300">
                          ...
                          <button 
                            ...
                            className="mt-8 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 text-[14px] font-bold text-white transition-all hover:bg-emerald-500 hover:shadow-lg hover:shadow-emerald-500/30 active:scale-[0.98] disabled:opacity-50"
                          >
                            {bindWalletMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Link2 className="h-4 w-4" />}
                            Aktivasi Profil Sekarang
                          </button>
                        </div>
                      </div>
                    )}

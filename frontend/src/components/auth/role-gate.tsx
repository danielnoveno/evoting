  ...
  if (!currentProfile || !allowedRoles.includes(currentProfile.role)) {
    return (
      <div className="rounded-2xl border border-red-200 bg-red-50 px-6 py-8 text-center text-red-800">
        ...
        <Link href={loginHref} className="mt-5 inline-flex h-10 items-center justify-center rounded-md bg-[#0F172A] px-5 text-[13px] font-medium text-white hover:bg-[#1E293B]">
          Kembali ke Login
        </Link>
      </div>
    )
